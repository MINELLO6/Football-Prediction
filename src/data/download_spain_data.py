...
def download_and_merge_data():
    ...
    dfs = []
    # Loop from 0 up to 24 (representing seasons from 0001 to 2425)
    for start in range(0, 25):
        season_code = f"{start:02d}{(start + 1) % 100:02d}"  # e.g., "0001" for 2000-2001, "2425" for 2024-2025
        url = f"https://www.football-data.co.uk/mmz4281/{season_code}/SP1.csv"  # Spanish La Liga is SP1
        print(f"Downloading data from: {url}")

        # Set the local path for the raw CSV file
        raw_file_path = os.path.join(raw_dir, f"SP1_{season_code}.csv")
    ...
    if dfs:
        ...
        merged_file_path = os.path.join(processed_dir, "merged_E1.csv")
